-- Phase 1 follow-up: property address, multi-building setup, no units required.
...
begin
  ...
  if agency_name is null or length(trim(agency_name)) = 0 then
    raise exception 'Security agency name is required';
  end if;
  ...
  insert into public.organizations (name, address, join_code, created_by, plan_id)
  values (trim(agency_name), trim(building_address), generated_code, auth.uid(), 'free')
